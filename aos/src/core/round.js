  ...
  modules.accounts.mergeAccountAndGet({
    ...
    async.series([
      ...
      function (cb) {
        ...
        library.logger.info('aos witness club get new bonus: ' + bonus)
...
Round.prototype.tick = function (block, cb) {
  ...
  modules.accounts.mergeAccountAndGet({
    ...
    if (private.delegatesByRound[round].length !== slots.delegates && block.height !== 1 && block.height !== 21) {
      return done();
    }
    ...
    async.series([
      ...
      function (cb) {
        ...
        // [HARDFORK] Need to be reviewed by aos community
        if (round === 33348) {
...
Round.prototype.onBlockchainReady = function () {
  ...
  library.dbLite.query("select sum(b.totalFee), GROUP_CONCAT(b.reward), GROUP_CONCAT(lower(hex(b.generatorPublicKey))) from blocks b where (select (cast(b.height / 21 as integer) + (case when b.height % 21 > 0 then 1 else 0 end))) = $round",
    {
      round: round
    },
    {
