...
var Sandbox = require('aos-sandbox');
...
const WITNESS_CLUB_DAPP_NAME = 'aos-community'
...
function DApp() {
  ...
  this.verify = function (trs, sender, cb) {
    ...
    if (!dapp.delegates || dapp.delegates.length < 5 || dapp.delegates.length > 21) {
  ...
  private.getInstalledIds(function (err, dappIds) {
    ...
    async.eachSeries(dappIds, function (id, next) {
      ...
      library.model.getDAppByName(WITNESS_CLUB_DAPP_NAME, function (err, clubInfo) {
        if (err) {
          library.logger.error('Failed to query aos witness club', err)
        } else if (!clubInfo) {
          library.logger.warn('aos witness club dapp is not found')
        } else {
