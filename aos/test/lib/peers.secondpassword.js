...
describe("POST /peer/transactions", function () {
    ...
    describe("Enabling second passphrase", function () {
        ...
        it("When accounts has no funds. Should fail", function (done) {
            var transaction = node.aossignature.createSignature(node.randomPassword(), node.randomPassword());
                    ...
                    node.api.put("/transactions")
                        ...
                        .end(function (err, res) {
                            ...
                            node.onNewBlock(function (err) {
                                ...
                                var transaction = node.aossignature.createSignature(account.password, account.secondPassword);
...
describe("POST /peer/transactions", function () {
    ...
    describe("Sending normal transaction with second passphrase now enabled", function () {
        ...
        it("When account doesn't have a second passphrase. Should fail", function (done) {
            var transaction = node.aostransaction.createTransaction("1", 1, node.Gaccount.password, account.secondPassword);
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version",node.version)
                .set("magic", node.config.magic)
        ...
        it("Using blank second signature. Should fail", function (done) {
            var transaction = node.aostransaction.createTransaction("1", 1, account.password, ""); // Send 1 XAS to address 1L
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
        ...
        it("Using fake second signature. Should fail", function (done) {
            var transaction = node.aostransaction.createTransaction("1", 1, account.password, account2.secondPassword); // Send 1 XAS to address 1L
            transaction.signSignature = crypto.randomBytes(64).toString("hex");
            transaction.id = node.aoscrypto.getId(transaction);
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
        ...
        it.skip("Using valid second signature. Should be ok", function (done) {
            var transaction = node.aostransaction.createTransaction("1", 1, account.password, account.secondPassword); // Send 1 XAS to address 1L
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
