...
describe("POST /peer/transactions", function () {
    ...
    before(function (done) {
        node.api.get("/delegates/")
            ...
            .end(function (err, res) {
                ...
                var transaction = node.aos.vote.createVote(votes, voterAccount.password);
    ...
    it("Voting twice for a delegate. Should fail", function (done) {
        node.onNewBlock(function (err) {
            var transaction = node.aos.vote.createVote(["+"+delegate1], voterAccount.password);
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
    ...
    it("Removing votes from a delegate and then voting again. Should fail", function (done) {
        node.onNewBlock(function (err) {
            var transaction = node.aos.vote.createVote(["-"+delegate2], voterAccount.password);
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
                ...
                .end(function (err, res) {
                    ...
                    node.expect(res.body).to.have.property("success").to.be.true;
                    var transaction2 = node.aos.vote.createVote(["+"+delegate2], voterAccount.password);
                    node.peer.post("/transactions")
                        .set("Accept", "application/json")
                        .set("version", node.version)
                        .set("magic", node.config.magic)
                ...
                node.api.put("/transactions")
                    ...
                    .end(function (err, res) {
                        node.onNewBlock(function (err) {
                            ...
                            var transaction = node.aos.delegate.createDelegate(account.username, account.password);
                            node.peer.post("/transactions")
                                .set("Accept", "application/json")
                                .set("version",node.version)
                                .set("magic", node.config.magic)
    ...
    it("Voting for a delegate. Should be ok", function (done) {
        var transaction = node.aos.vote.createVote(["+" + account.publicKey], account.password);
        node.onNewBlock(function (err) {
            node.expect(err).to.be.not.ok;
            node.peer.post("/transactions")
                .set("Accept", "application/json")
