angular.module('aos').service('nodeService', function ($http) {
    ...
    aosServer.compareServer = function(server1, server2){
    ...
    function registerServer(serverUrl){
        if (findServerIndex(serverUrl) < 0){
            var server = new aosServer(serverUrl);
    ...
    function sortServers(serverArray){
        serverArray = serverArray || servers;
        return serverArray.sort(aosServer.compareServer);
    }
    ...
    function adjustClientDriftSeconds(server){
        ...
        aosJS.options.set('clientDriftSeconds', timeAdjust);
    }
