angular.module('aos').controller('personalCtrl', function ($scope, $rootScope, apiService, ipCookie, $window, $http, userService, postSerivice, $translate) {
	...
	// $scope.string = "/aos/secret/raw/" + $rootScope.qrcode;
	...
	$scope.createTrsPsd = function() {
		return aosJS.signature.createSignature(userService.secret, $scope.secondpassword);
	}

	$scope.createTrsLok = function() {
		var lockHeight = Number($scope.block_number);
		return aosJS.transaction.createLock(lockHeight, userService.secret, $scope.secondpassword);
	}
