angular.module('aos').controller('loginCtrl', function ($scope, $rootScope, apiService, ipCookie, $window, $location, userService, $translate) {
	...
	$scope.newuser = function () {
		...
		newpublicKey = aosJS.crypto.getKeys($scope.newsecret).publicKey;
	...
	$scope.saveTxt = function (filename) {
		...
		var address = aosJS.crypto.getAddress(newpublicKey);
		txt = 'secret:' + '\r\n' + text + '\r\n\r\n' + 'address:' + '\r\n' + address + '\r\n';
	...
	$scope.registerin = function () {
		...
		var publicKey = aosJS.crypto.getKeys($scope.secret).publicKey;
		// 增加root qrstr属性git
