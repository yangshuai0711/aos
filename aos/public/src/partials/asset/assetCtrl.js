 ...
 */
angular.module('aos').controller('assetCtrl', function ($scope, $rootScope, apiService, ipCookie, $location, $window, NgTableParams,userService,postSerivice, $translate,$uibModal) {
    ...
    $scope.registerPublish = function () {
        ...
        $scope.publishtrs = aosJS.uia.createIssuer(name, desc, userService.secret, $scope.rpsecondPassword);
    ...
    $scope.registerAsset = function () {
        ...
        $scope.assetTrs = aosJS.uia.createAsset(String(name), String(desc), String(realMaximum), precision, strategy, allowWriteoff, allowWhitelist, allowBlacklist, userService.secret, $scope.rasecondPassword);
        $scope.dialogNUM = 2;
        $scope.comfirmDialog = true;
        $rootScope.isBodyMask = true;
    };
    ...
    $scope.writeoff_submit = function () {
        ...
        var transaction = aosJS.uia.createFlags(currency, flagType, flag,userService.secret, $scope.wosecondPassword);
    ...
    $scope.publish_submit = function () {
        ...
        var trs = aosJS.uia.createIssue($scope.myPublishmoneyName, String(realAmount), userService.secret, $scope.pbsecondPassword);
    ...
    $scope.settings_submit = function () {
        ...
        var trs = aosJS.uia.createFlags(currency, flagType, flag, userService.secret, $scope.setsecondPassword);
        postSerivice.writeoff(trs).success(function (res) {
            if (res.success == true) {
                $scope.setsecondPassword = '';
                $scope.myAss.set = false;
