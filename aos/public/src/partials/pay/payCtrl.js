angular.module('aos').controller('payCtrl', function ($scope, $rootScope, $filter, apiService, ipCookie, $http, $window, userService, postSerivice, $translate) {
    ...
    $scope.calculateFee = function () {
        if ($scope.amount && Number($scope.amount) > 0) {
            ...
            var fee = aosJS.transaction.calculateFee(amount);
    ...
    $scope.createTransaction = function() {
        ...
        if(!$rootScope.currencyName){
            return aosJS.transaction.createTransaction(String($scope.fromto), amount, message, userService.secret, $scope.secondPassword);
        } else {
            amount = ($scope.amount*Math.pow(10, $rootScope.precision)).toFixed(0);
            return aosJS.uia.createTransfer(String($rootScope.currencyName), amount, String($scope.fromto), message, userService.secret, $scope.secondPassword)
        }
    }
    $scope.sentMsg = function () {
        ...
        if(!$rootScope.currencyName){
            ...
            //transaction = aosJS.transaction.createTransaction(String($scope.fromto), amount, message, userService.secret, $scope.secondPassword);
        } else {
            amount = ($scope.amount*Math.pow(10, $rootScope.precision)).toFixed(0);
            //transaction = aosJS.uia.createTransfer(String($rootScope.currencyName), amount, String($scope.fromto), message, userService.secret, $scope.secondPassword)
        }
